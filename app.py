...
def create_app(dp: Dispatcher, bot: Bot) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        # Определяем, будем ли мы использовать webhook
        # Можно сделать более гибкую настройку, например, через env var
        use_webhook = is_render

        task = None
        if use_webhook:
            # На Render (или когда webhook включен) НЕ запускаем дополнительный polling
            # Webhook устанавливается в main.py
            print("🤖 Telegram бот готов к работе через Webhook.")
        else:
